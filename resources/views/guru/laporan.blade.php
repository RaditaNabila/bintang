...
<div class="space-y-6">
    ...
    <!-- Filter Controls Bar -->
    <form method="GET" action="{{ route('guru.laporan') }}" class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex flex-wrap items-center gap-3">
        ...
        <div class="flex items-center gap-2 bg-gray-50 px-3 py-2 border border-gray-200 rounded-xl">
            ...
            >
                <option value="all" {{ request('kelas_id') == 'all' ? 'selected' : '' }}>Semua Kelas</option>
                @foreach($kelas as $k)
        ...
        <button type="submit" class="bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold px-4 py-2.5 rounded-xl transition">
            Cari
        </button>

        @if(request('search') || request('kelas_id') != 'all')
            <a href="{{ route('guru.laporan') }}" class="text-xs text-gray-500 hover:text-gray-700 font-medium px-2 py-2">
                Reset Filter
            </a>
        @endif
    </form>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-100 pb-4">
            <div>
                <h3 class="text-base font-bold text-gray-800">Daftar Sisa Poin Siswa</h3>
                <p class="text-xs text-gray-500">Rekap saldo akhir poin kedisiplinan dan karakter siswa</p>
            </div>
                        ...
                        <tr class="hover:bg-gray-50/50">
                            ...
                            <td class="py-3.5 px-4 text-center">
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl font-extrabold text-base border {{ $poinBadge }}">
                                    <i class="fa-solid fa-star text-xs"></i> {{ $poin }}
                                </span>
                            </td>
                            <td class="py-3.5 px-4 text-center">
